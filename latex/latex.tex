...
原理:
\\
1.對於一個在理想流體中平移的理想球體，其附加質量為排開流體質量的一半：
$$m_a = C_m \rho_w V$$
...
由於擺錘的密度 $\rho$ 與水的密度 $\rho_w$已知，量測同一個擺錘在空氣中與水中的週期 $(T_{water}$、$T_{air})$，取其線性回歸斜率，就能直接計算出 $C_m$，並可與已知物理模型比較與驗證。
